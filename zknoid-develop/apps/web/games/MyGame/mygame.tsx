...
import { mynumberGuessingConfig } from './config';
...
export default function NumberGuessing({
  ...
  const client_ = client as ClientAppChain<
    typeof mynumberGuessingConfig.runtimeModules,
    any,
    any,
    any
  >;
  ...
  const query = networkStore.protokitClientStarted
    ? client_.query.runtime.MyGame
    : undefined;

  const hideNumber = async (number: number) => {
    const guessLogic = client_.runtime.resolve('MyGame');
  ...
  return (
    <GamePage
      gameConfig={mynumberGuessingConfig}
      image={CoverSVG}
